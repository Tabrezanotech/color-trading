...
class Dashboard1Controller extends Controller
{
    public function index()
    {
        ...
        // Find active 1-minute round
        $round = GameRound::where('start_time', '<=', $now)
                          ->where('end_time', '>', $now)
                          ->where('duration_type', '1m')
                          ->first();

        // If no round exists, create a new 1-minute round
        if (!$round) {
            $last = GameRound::where('duration_type', '1m')->latest('counter_id')->first();
            $newCounterId = $last ? $last->counter_id + 1 : 11230001;

            $start = $now->copy()->startOfSecond();
            $end = $start->copy()->addSeconds(60); // ⏱ 1 minute
    ...
    {
        $round = GameRound::where('duration_type', '1m')
                          ->latest('id')
                          ->first();
