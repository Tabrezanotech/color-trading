...
class Dashboard3Controller extends Controller
{
    public function index()
    {
        ...
        // Find active 3-minute round
        $round = GameRound::where('start_time', '<=', $now)
                          ->where('end_time', '>', $now)
                          ->where('duration_type', '3m')
                          ->first();

        // If no round exists, create a new 3-minute round
        if (!$round) {
            $last = GameRound::where('duration_type', '3m')->latest('counter_id')->first();
            $newCounterId = $last ? $last->counter_id + 1 : 11230001;

            $start = $now->copy()->startOfSecond();
            $end = $start->copy()->addMinutes(3); // ⏱ 3 minutes
    ...
    {
        $round = GameRound::where('duration_type', '3m')
                          ->latest('id')
                          ->first();
